...
def generate_sample_data():
    ...
    categories = [
        # Électroménager (gros appareils)
        ("electro_menager", "machine_a_laver", None, 1.0),
        ("electro_menager", "lave_vaisselle", None, 1.0),
        ("electro_menager", "refrigerateur", None, 1.0),
        ("electro_menager", "congelateur", None, 1.0),
        ("electro_menager", "four", None, 1.0),
        ("electro_menager", "plaque_induction", None, 1.0),
        ("electro_menager", "micro_ondes", None, 1.0),
        ("electro_menager", "seche_linge", None, 1.0),
        ("electro_menager", "lave_linge_sechant", None, 1.1),
        
        # Multimédia
        ("multi_media", "television", None, 1.2),
        ("multi_media", "ordinateur_portable", None, 1.2),
        ("multi_media", "ordinateur_fixe", None, 1.2),
        ("multi_media", "tablette", None, 1.2),
        ("multi_media", "smartphone", None, 1.2),
        ("multi_media", "console_jeux", None, 1.3),
        ("multi_media", "enceinte_connectee", None, 1.2),
        ("multi_media", "casque_audio", None, 1.1),
        ("multi_media", "appareil_photo_numerique", None, 1.2),
        ("multi_media", "camera_video", None, 1.2),
        
        # Petit électro
        ("petit_electro", "cafetiere", None, 1.0),
        ("petit_electro", "bouilloire_electrique", None, 1.0),
        ("petit_electro", "grille_pain", None, 1.0),
        ("petit_electro", "mixeur", None, 1.0),
        ("petit_electro", "robot_cuisine", None, 1.1),
        ("petit_electro", "aspirateur", None, 1.1),
        ("petit_electro", "fer_a_repasser", None, 1.0),
        ("petit_electro", "machine_expresso", None, 1.1),
    ]
...
for i in range(1, 11):
    products.append((
        f"SM{i:03d}", f"EAN13{random.randint(1000000000000, 9999999999999)}",
        f"Smartphone Pro {i}", f"Smartphone {random.choice(['compact', 'écran large', 'performant'])}, {random.choice(['128Go', '256Go', '512Go'])} de stockage, appareil photo {random.choice(['triple', 'quadruple'])} capteurs, coloris {random.choice(['noir', 'blanc', 'bleu', 'vert'])}",
        6, None, "MaMarque", round(random.uniform(399.99, 999.99), 2), 1.2, "actif", None
    ))

    # Ordinateurs Portables
    for i in range(1, 8):
        products.append((
            f"OR{i:03d}", f"EAN13{random.randint(1000000000000, 9999999999999)}",
            f"Ordinateur Portable {i}", f"Laptop {random.choice(['bureautique', 'gaming', 'création'])} avec processeur {random.choice(['Intel Core i5', 'Intel Core i7', 'AMD Ryzen 5', 'AMD Ryzen 7'])}, RAM {random.choice(['8Go', '16Go', '32Go'])}, SSD {random.choice(['256Go', '512Go', '1To'])}",
            6, None, "MaMarque", round(random.uniform(599.99, 1499.99), 2), 1.3, "actif", None
        ))

    # Tablettes
    for i in range(1, 6):
        products.append((
            f"TB{i:03d}", f"EAN13{random.randint(1000000000000, 9999999999999)}",
            f"Tablette Multimédia {i}", f"Tablette {random.choice(['compact', 'écran large'])}, {random.choice(['64Go', '128Go', '256Go'])} de stockage, processeur {random.choice(['haute performance', 'ultra rapide'])}, coloris {random.choice(['gris', 'noir', 'bleu'])}",
            7, None, "MaMarque", round(random.uniform(249.99, 699.99), 2), 1.2, "actif", None
        ))

    # Télévisions
    for i in range(1, 8):
        products.append((
            f"TV{i:03d}", f"EAN13{random.randint(1000000000000, 9999999999999)}",
            f"Téléviseur {i}", f"TV {random.choice(['4K', '8K'])} {random.choice(['50', '55', '65', '75'])} pouces, technologie {random.choice(['OLED', 'QLED', 'LED'])}, smart TV, HDR {random.choice(['Premium', 'Dolby Vision'])}, son {random.choice(['Dolby Atmos', 'DTS:X'])}",
            8, None, "MaMarque", round(random.uniform(499.99, 2499.99), 2), 1.3, "actif", None
        ))

    # Consoles de Jeux
    for i in range(1, 5):
        products.append((
            f"CJ{i:03d}", f"EAN13{random.randint(1000000000000, 9999999999999)}",
            f"Console Gaming {i}", f"Console de jeux {random.choice(['nouvelle génération', 'performance premium'])}, stockage {random.choice(['512Go', '1To'])}, édition {random.choice(['standard', 'deluxe'])}",
            9, None, "MaMarque", round(random.uniform(299.99, 599.99), 2), 1.3, "actif", None
        ))
    ...
    for i in range(1, 5):
        products.append((
            f"CS{i:03d}", f"EAN13{random.randint(1000000000000, 9999999999999)}",
            f"Casque Audio {i}", f"Casque {random.choice(['sans fil', 'à réduction de bruit', 'gaming'])} avec technologie {random.choice(['Bluetooth 5.0', 'audio haute définition'])}, autonomie {random.choice(['jusqu\'à 20h', 'jusqu\'à 30h'])}, coloris {random.choice(['noir', 'blanc', 'gris'])}",
            11, None, "MaMarque", round(random.uniform(99.99, 349.99), 2), 1.1, "actif", None
        ))

    # Appareils Photo et Vidéo
    for i in range(1, 4):
        products.append((
            f"AP{i:03d}", f"EAN13{random.randint(1000000000000, 9999999999999)}",
            f"Caméra Professionnelle {i}", f"Caméra {random.choice(['4K', '8K'])}, capteur {random.choice(['plein format', 'APS-C'])}, stabilisation {random.choice(['intégrée', 'optique'])}, enregistrement {random.choice(['vidéo professionnelle', 'slow motion'])}",
            12, None, "MaMarque", round(random.uniform(799.99, 2999.99), 2), 1.5, "actif", None
        ))

    # Produit High-Tech Innovant
    products.append((
        "HT001", f"EAN13{random.randint(1000000000000, 9999999999999)}",
        "Drone Technologique", "Drone professionnel avec caméra 4K, autonomie de vol prolongée, système de stabilisation avancé, plage de vol jusqu'à 5km",
        15, None, "MaMarque", round(random.uniform(999.99, 2499.99), 2), 2.0, "actif", None
    ))
